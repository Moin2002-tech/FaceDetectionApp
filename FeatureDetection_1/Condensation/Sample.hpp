...
* Author : Moin Shaikh
*/
#ifndef FEATUREDETECTION_CONDENSATION_SAMPLE_HPP_
#define FEATUREDETECTION_CONDENSATION_SAMPLE_HPP_
...
{	
	...
	*/
	...
	public:
		...
		*/
		Sample() :  x(0), y(0), size(0),vx(0),vy(0),vsize(1),
			weight(1), score(0), target(false), clusterId(getNextClusterId()), ancestor(nullptr) {}
		...
		*/
		explicit Sample(std::shared_ptr<Sample> others, double weight = 0) : x(others->x), y(others->y), size(others->size), vx(others->vx), vy(others->vy),
			vsize(others->vsize), weight(weight), score(0), target(false), clusterId(-1), ancestor(others)
		{
	 ...
	 */
		...
		*/
		...
		class WeightComparisonAscending 
		{
		public:
		...
		*/
		...
		class WeightComparisonDescending
		{
		public:
			bool operator()(const Sample& lhs, const Sample& rhs) const
			{
...
#endif //FEATUREDETECTION_CONDENSATION_SAMPLE_HPP_