...
#include "../ImageProcessing/HistoEq64Filter.hpp"
#include "../ImageProcessing/WhiteningFilter.hpp"
#include "../ImageProcessing/ZeroMeanVarianceFilter.hpp"
#include "../ImageProcessing/HistogramEquilizationFilter.hpp"
...
#include "../Classification/FixedTrainableProbabilisticClassifier.hpp"
#include "../svm/LibSvmClassifier.hpp"
...
shared_ptr<kernel> PartiallyAdaptiveTracking::createKernel(ptree config)
{
	...
	{
		return make_shared<PolynomialKernel>(config.get<double>("constant", config.get<double>("degree")));
	}
...
shared_ptr<TrainableSvmClassifier> PartiallyAdaptiveTracking::createTrainableSvm(shared_ptr<kernel> kernel, ptree config)
{
	...
	if (config.get_value<string>() == "fixedsize") 
	{
		...
		trainableSvm->setPositiveExampleManagement(unique_ptr<ExampleManagement>(
			new ConfidenceBasedExampleManagement(trainableSvm, config.get<size_t>("positiveExamples"),
				config.get<size_t>("minPositiveExamples"))));
...
void PartiallyAdaptiveTracking::initGui() {
	...
	cv::namedWindow(videoWindowName.c_str(), cv::WINDOW_AUTOSIZE);
	cv::moveWindow(videoWindowName.c_str(), 50, 50);

	cv::namedWindow(controlWindowName.c_str(), cv::WINDOW_AUTOSIZE);
	cv::moveWindow(controlWindowName.c_str(), 900, 50);
...
{
	Logger& log = LoggerFactory::Instance()->getLogger("app");
	running = true;
	paused = false;
...
int main(int argc, char* argv[]) {
	...
	int deviceId = 0, kinectId;
	string filename, directory;
	bool useCamera = true, useKinect = false, useFile = false, useDirectory = false;
	string configFile;
	string outputFile;
	int outputFps = -1;
	...
	//getLogger("app").addAppender(make_shared<ConsoleAppender>(LogLevel::Info));
