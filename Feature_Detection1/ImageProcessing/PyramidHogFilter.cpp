/*
* File: PyramidHogFilter.cpp
* Created on: 15/2/2025
* Author : Moin Shaikh
*/

#include"PyramidHogFilter.hpp"
...
namespace ImageProcessing
{
	...
	PyramidHogFilter::PyramidHogFilter(int binCount, int levelCount, 
		bool interpolate, bool signedAndUnsigned) :
		
		binCount(binCount), maxLevel(levelCount - 1),
		interpolate(interpolate),
		signedAndUnsigned(signedAndUnsigned), 
		HistogramFilter(Normalization::L2NORM), histogramCount(0)
	{
		if (binCount <= 0)
	...
	void PyramidHogFilter::createCellHistograms(const Mat& image, float* histogramsValues,int level, 
		int binCount, bool interpolate,
		bool signedAndUnsigned) const
	{
	...
	void PyramidHogFilter::copyCellHistograms(const float* source, float* destination,
		int binCount, int cellCount, bool signedAndUnsigned) const
	{
	...
	void PyramidHogFilter::combineHistograms(const float* cellHistogramsValues,
		float* blockHistogramsValues, int level, int binCount) const
	{
	...
	void PyramidHogFilter::normalizeHistograms(float* histogramsValues, int histogramCount, 
		int binCount, bool signedAndUnsigned) const
	{
		int binHalfCount = binCount / 2;
