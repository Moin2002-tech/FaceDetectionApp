...
namespace ImageProcessing
{
	...
	shared_ptr<ImagePyramid> ImagePyramid::create(int octaveLayerCount,
	...
	shared_ptr<ImagePyramid> ImagePyramid::createFiltered(shared_ptr<ImagePyramid> pyramid, 
		const shared_ptr<ImageFilter>& filter) 
	{
	...
	void ImagePyramid::update() {
		...
		else if (sourcePyramid) 
		{
			...
			LoggerFactory::Instance()->getLogger("ImageProcessing").warn("ImagePyramid: could not update because there is no source (image or pyramid)");
		}
	}
	...
	vector<double> ImagePyramid::computeChannelRatios(const vector<double>& channelMeans1,
		const vector<double>& channelMeans2) const
	{
