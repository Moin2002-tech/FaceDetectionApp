...
#ifndef FEATUREDETECTION_IMAGEPROCESSING_PATCH_HPP_
#define FEATUREDETECTION_IMAGEPROCESSING_PATCH_HPP_
...
namespace ImageProcessing
{
	...
	*/
	...
	public:
		...
		*/
		Patch(int x, int y, int width, int height, const cv::Mat& data) : center(x, y), size(width, height), data(data)
		{
		...
		*/
		static cv::Rect computeBounding(cv::Point center, cv::Size size)
		{
			return cv::Rect(center.x - size.width/ 2,
		...
		*/
		...
		{
			return computeBounding(center, size);
		}
		 ...
		 */
		 // : std::unary_function<Patch, size_t>
		struct hash {
...
#endif //FEATUREDETECTION_IMAGEPROCESSING_PATCH_HPP_