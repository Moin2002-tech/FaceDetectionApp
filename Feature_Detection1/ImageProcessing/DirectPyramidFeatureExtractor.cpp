/*
* File: DirectPyramidFeatureExtractor.hpp
* Created on: 20/2/2025
* Author : Moin Shaikh
*/


#include "DirectPyramidFeatureExtractor.hpp"
#include "ImagePyramid.hpp"
#include "ImagePyramidLayer.hpp"
#include "Patch.hpp"
...
{
	shared_ptr<ImagePyramid> DirectPyramidFeatureExtractor::createPyramid(int width, int minWidth, 
		int maxWidth, int octaveLayerCount) 
	{
	...
	DirectPyramidFeatureExtractor::DirectPyramidFeatureExtractor(int width, int height,
		int minWidth, int maxWidth, int octaveLayerCount) :
	...
	DirectPyramidFeatureExtractor::DirectPyramidFeatureExtractor(int width, int height, int minWidth,
		int maxWidth, double incrementalScaleFactor) :
		pyramid(createPyramid(width, minWidth, maxWidth,
			static_cast<size_t>(std::round(log(0.5) / log(incrementalScaleFactor))))),
		patchWidth(width), patchHeight(height), patchFilter(make_shared<ChainedFilter>())
	{
