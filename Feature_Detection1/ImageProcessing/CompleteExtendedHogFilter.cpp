/*
* File: CompleteExtendedHogFilter.hpp
* Created on: 20/2/2025
* Author : Moin Shaikh
*/


#include"CompleteExtendedHogFilter.hpp"
#include<stdexcept>
...
namespace ImageProcessing
{
	...
	CompleteExtendedHogFilter::CompleteExtendedHogFilter(size_t cellSize, size_t binCount, 
		bool signedGradients, bool unsignedGradients,
	...
	void CompleteExtendedHogFilter::buildDescriptors(Mat& descriptors, size_t cellRowCount, 
		size_t cellColumnCount, size_t descriptorSize) const
	{
		...
		if (signedGradients && unsignedGradients)
		{ // signed and unsigned gradients should be combined into descriptor
			for (size_t rowIndex = 0; rowIndex < cellRowCount; ++rowIndex) 
			{
				for (size_t colIndex = 0; colIndex < cellColumnCount; ++colIndex)
				{
		...
		{ // only signed or unsigned gradients should be in descriptor
			for (size_t rowIndex = 0; rowIndex < cellRowCount; ++rowIndex) 
			{
				for (size_t colIndex = 0; colIndex < cellColumnCount; ++colIndex) 
				{
