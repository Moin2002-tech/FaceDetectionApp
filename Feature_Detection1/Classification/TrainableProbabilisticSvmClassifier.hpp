...
#ifndef FEATUREDETECTION_CLASSIFICATION_TRAINABLE_PROBABILISITC_CLASSIFIER_HPP_
#define FEATUREDETECTION_CLASSIFICATION_TRAINABLE_PROBABILISITC_CLASSIFIER_HPP_
...
namespace Classification 
{
	...
	*/
	...
	public:
		...
		*/
		TrainableProbabilisticSvmClassifier(std::shared_ptr<TrainableSvmClassifier> trainableSvm,
			int postiveCount, int negativeCount,
			double highProbability = 0.99, double lowProbability = 0.01);
		...
		bool retrain(const std::vector<cv::Mat>& newPostiveExample, const std::vector<cv::Mat>& newNegativeExample);
	...
	protected:
		...
		*/
		void addTestExample(std::vector<cv::Mat>& example,
			const std::vector<cv::Mat>& newExample, size_t& intertedExample);
		/**
...
#endif //FEATUREDETECTION_CLASSIFICATION_TRAINABLE_PROBABILISITC_CLASSIFIER_HPP_