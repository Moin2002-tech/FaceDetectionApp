...
namespace Render {
	...
	void Camera::updateFixed(const Vec3f& eyes, const Vec3f& gaze, const Vec3f& up)
	{
		this->eye = eye;
		this->gaze = gaze;
		this->up = up;
	...
	{
		Mat transformMatrix = Render::utils::MatrixUtils::CreateRotationMatrix_Y(horizontalAngle) * Render::utils::MatrixUtils::createRotationMatrix_X(verticalAngle);
		Mat tmpRes = transformMatrix * Mat(Vec3f(0.0f, 1.0f, 1.0f));
		...
		Mat tmpRes2 = transformMatrix * Mat(cv::Vec4f(1.0f, 0.0f, 0.0f, 1.0f));
